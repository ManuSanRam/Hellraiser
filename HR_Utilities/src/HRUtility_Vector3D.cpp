...
namespace HR_SDK
{
	...
	*/
	...
	{
		//! Assign values to the vector's elements
		m_x = prm_X;
		m_y = prm_Y;
		m_z = prm_Z;
	}
	...
	*/
	C_Vector3D	C_Vector3D::Cross(const C_Vector3D& prm_Vector)
	{
		return C_Vector3D
		(
			(m_y * prm_Vector.m_z) - (m_z * prm_Vector.m_y),  //! Get X
			(m_z * prm_Vector.m_x) - (m_x * prm_Vector.m_z),  //! Get Y
			(m_x * prm_Vector.m_y) - (m_y * prm_Vector.m_x)   //! Get Z
		);
	}
